...
import { z } from "zod";
import { uploadImageViaPlaywright } from "./playwright-upload.server";
  ...
  .handler(async ({ data }) => {
    ...
    try {
      ...
      if (token) {
        ...
        if (hash) return { success: true, imageHash: hash, error: null, capturedAv: null, capturedDynParams: null };
        attempts.push({
          name: "graph-api",
    ...
    // Attempt 3: Playwright – launches a real headless Chromium, injects the
    // user's cookies, navigates to Ads Manager so the real Comet/Relay app fires
    // its first GraphQL request, intercepts it to harvest ALL live session params
    // (fb_dtsg, lsd, av, __dyn, __csr, __s, __hsi, __aaid, …), then fires the
    // LWICometAdAccountUploadImageMutation via server-side fetch with those params.
    //
    // This fixes both prior failures:
    //   [graph-api 0] No access token in adsmanager HTML
    //      → adsmanager is a CSR app; the EAA token is never in server-fetched HTML.
    //   [graphql 200] 1357001
    //      → FB rejects Comet requests missing the heavy session params.
    try {
      const pw = await uploadImageViaPlaywright({
        ...
        width: data.width,
        height: data.height,
        docId: c.graphqlDocuments?.[UPLOAD_IMAGE_MUTATION] ?? UPLOAD_IMAGE_DOC_ID,
      });
      if (pw.imageHash) {
        // Return captured session params so the caller can pass them to createAd
        // without opening another browser (set credentials.av + credentials.dynParams).
        return {
          success: true,
          imageHash: pw.imageHash,
          error: null,
          capturedAv: pw.session?.av ?? null,
          capturedDynParams: pw.session?.dynParams ?? null,
        };
      }
      attempts.push({
        name: "playwright",
        status: 0,
        body: pw.error?.slice(0, 400) ?? "no hash in response",
      });
    } catch (error: unknown) {
      attempts.push({ name: "playwright", status: 0, body: errorMessage(error) });
    }
